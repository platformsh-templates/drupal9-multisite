...
This template builds Drupal 9 in a multisite configuration using the "Drupal Recommended" Composer project.  It is pre-configured to use MariaDB and Redis for caching.  The Drupal installer will skip asking for database credentials as they are already provided.
...
| [`web/sites/default/settings.php`](web/sites/default/settings.php) | **Added:**<br><br> The Drupal settings file has been updated to import and use `web/sites/settings.platformsh.php`. You will also need an identical file for each subsite (i.e. `first`, `second`). |
| [`web/sites/default/services.yml`](web/sites/default/services.yml) | **Added:**<br><br> Default services configuration. You will also need an identical file for each subsite (i.e. `first`, `second`). |
| [`web/sites/default/drushrc.php`](web/sites/default/drushrc.php) | **Added:**<br><br> Drush configuration file for a Platform.sh Drupal site. You will also need an identical file for each subsite (i.e. `first`, `second`). |
